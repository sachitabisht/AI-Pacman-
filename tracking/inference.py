...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder:
            cpts, join_factor = joinFactorsByVariable(cpts, variable)
            if len(join_factor.unconditionedVariables()) == 1:
                continue    
            join_factor = eliminate(join_factor, variable)
            cpts.append(join_factor)
        normal_factor = normalize(joinFactors(cpts))
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        if total_value != 0:
            for keys in self:
                self[keys] = self[keys]/total_value
       #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
    ...
    def sample(self):
        ...
        for i in list(self.items()):
            prob += (i[1] / total_value)
            if prob >= randomSample:
                return i[0]
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for p in self.allPositions:
            self.beliefs[p] *= self.getObservationProb(observation, pacPos, p, jailPos)
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for p in self.allPositions:
            ...
            for pd in posDis.items():
                discDist[pd[0]] += self.beliefs[p] * posDis[pd[0]]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        num = 0
        legPos = len(self.legalPositions)
        for p in range(self.numParticles):
            if num >= legPos:
                num = 0
            self.particles.append(self.legalPositions[num])
            num += 1
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        for p in self.particles:
            discDist[p] = self.particles.count(p)
        discDist.normalize()
        return discDist
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        discDist = DiscreteDistribution()
        for p in self.particles:
            obsProb = self.getObservationProb(observation, pacPos, p, jailPos)
            discDist[p] += obsProb
        discDist.normalize()
        #special case
        if discDist.total() == 0:
            ...
            sampleDist = [discDist.sample() for _ in range(len(self.particles))]
            self.particles = sampleDist
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            posDis = self.getPositionDistribution(gameState, p)
            particleState.append(posDis.sample())

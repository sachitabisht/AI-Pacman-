    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        ghosts_positions = [max(dist, key=dist.get) for dist in livingGhostPositionDistributions]
        closest_distance = float('inf')
        best_ghost_position = None
        for ghost_position in ghosts_positions:
            distance_to_ghost = self.distancer.getDistance(pacmanPosition, ghost_position)
            if distance_to_ghost < closest_distance:
                closest_distance = distance_to_ghost
                best_ghost_position = ghost_position
        best_action = None
        for action in legal:
            successor_position = Actions.getSuccessor(pacmanPosition, action)
            if self.distancer.getDistance(successor_position, best_ghost_position) < closest_distance:
                best_action = action
        return best_action
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
